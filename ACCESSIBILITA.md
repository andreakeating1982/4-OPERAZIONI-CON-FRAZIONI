# ♿ ACCESSIBILITÀ — Operazioni con le Frazioni
...
> un'altra app" è in fondo a questa pagina e in `GUIDA-IA.md` (sezione 11).

> Misure di accessibilità/inclusione aggiunte all'app **OPERAZIONI CON LE FRAZIONI**
> (ex "Math Input Panel"), portate dall'app di riferimento **LATINO FACILE** /
> **EQUAZIONI DI QUARTO GRADO BIQUADRATICHE**
> (https://github.com/andreakeating1982/EQUAZIONI-DI-QUARTO-GRADO-TRINOMIE-BIQUADRATICHE).
...
| 10 | **ARIA, tastiera e input alternativo** (`✎ digita il valore`) | `AccessibilityToolbar.tsx`, `WelcomePage.tsx`, `NumberInputCanvas.tsx` |
...
- File auto-ospitati in `client/public/fonts/` (TTF, OTF, WOFF2 + versioni `-v2`).
  Nessun CDN esterno.
...
- **La notazione matematica (KaTeX) conserva il proprio font** (KaTeX_Main ecc.), non
  sovrascritto: le frazioni e le formule restano perfettamente allineate.
...
- **converte le formule KaTeX dal LaTeX in italiano parlato**: frazioni ("fratto"),
  esponenti ("x⁴" → "x alla quarta", "x²" → "x al quadrato"), radici ("radice quadrata di"),
  "più o meno", "delta";
...
### ARIA e struttura
- Le pagine usano il landmark `<main>` (WelcomePage e FractionExercises) per la
...
- Il canvas di scrittura a mano è affiancato dal pulsante **✎ digita il valore**
  (input da tastiera, `type="text"` + `onKeyDown` per invio), alternativa per chi non
  può usare il tratto.

### PDF
Il PDF del quaderno (`handleScaricaPdf`) ora usa **OpenDyslexic** (con `<base>` e
`@font-face` inline nel documento di stampa, file `/fonts/OpenDyslexic-Regular.ttf`
e `-Bold.ttf`), mantenendo il fallback Cambria Math per le formule KaTeX.
...
- [ ] Il pulsante **✎ digita il valore** permette di inserire le cifre da tastiera.
