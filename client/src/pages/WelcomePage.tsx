...
export default function WelcomePage() {
  ...
  // ─── Margini prima pagina (come LATINO FACILE) ───────────────────
  // Vista autonoma (fuori dall'iframe del blog): la pagina deve
  // "terminare" subito sotto la card, senza il grande vuoto crema.
  // Aggiunge la classe lf-welcome-top su <html> che rende neutro
  // (bianco) il canvas sotto il contenuto. Dentro la cornice (iframe)
  // NON viene aggiunta: lì è heightSync.ts a segnalare l'altezza reale
  // e la cornice dinamica si restringe da sola.
  useEffect(() => {
  ...
  return (
    <main className="lf-welcome flex flex-col items-center bg-background px-4 pb-8 sm:pb-12">
      {/* Layout compatto: la card sta subito sotto la barra di accessibilità,
          con un piccolo respiro sopra e sotto, e la pagina termina subito
          dopo la card (niente più grande spazio vuoto sopra/sotto). */}
      <div className="flex w-full flex-col items-center pt-4 sm:pt-8">
        ...
        <p className="text-[11px] text-muted-foreground mb-4 tracking-wide">
          ...
          <span style={{ color: "#8B3A1A", fontWeight: 600 }}>PASSO DOPO PASSO</span>
        </p>
