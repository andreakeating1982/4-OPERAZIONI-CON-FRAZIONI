# CORNICE DINAMICA — OPERAZIONI CON LE FRAZIONI (embed per Blogger)

Questa cartella contiene la **cornice dinamica** dell'app OPERAZIONI CON LE FRAZIONI:
un blocco HTML autonomo da incollare su Blogger (o su qualsiasi sito)
...
| `embed-frazioni-lite.html` | **Versione leggera (~5 KB)**: font OpenDyslexic caricati dall'app via CORS. Codice piccolo e leggibile, ideale da incollare nel post. **Riutilizzabile**: per un'altra app basta cambiare la riga `APP_URL` (o passare `?app=URL` nella pagina). Richiede che l'app sia online. |
...
| `embed-frazioni.html` | **Versione autosufficiente (~110 KB)**: font incorporati in base64. Funziona anche se l'app è offline (i font restano) ed è robusta su qualsiasi piattaforma. |
...
2. Incolla l'intero contenuto di `embed-frazioni-dedicata.html` (⭐ consigliata)
   oppure di `embed-frazioni-lite.html` (minima) o di
   `embed-frazioni.html` (autosufficiente).
...
| Caratteristica | `...-dedicata.html` | `...-lite.html` | `embed-frazioni.html` |
|---|---|---|---|
...
1. **Nuova app con la versione DEDICATA (Schermo intero + Ricarica + stato +
   impermeabile)**: parti da `embed-frazioni-dedicata.html`, cambia:
   - la `APP_URL` nella sezione `⚙️ CONFIGURAZIONE` (riga `var APP_URL = ...`);
   - i due URL dei font nel blocco `@font-face` in cima al file
     (`https://math-input-panel.easy-peasy.site/fonts/...` → l'URL della tua app);
   - (facoltativo) il testo del titolo `OPERAZIONI CON LE FRAZIONI`.
...
2. **URL dell'app (file esistenti)**: nei file HTML cerca
   `https://math-input-panel.easy-peasy.site` e sostituiscilo con l'URL di
...
- Nella versione dedicata il titolo è **centrato** e i pulsanti stanno **sotto
  il titolo, centrati**.
- Il titolo "OPERAZIONI CON LE FRAZIONI" è in **OpenDyslexic Bold**
