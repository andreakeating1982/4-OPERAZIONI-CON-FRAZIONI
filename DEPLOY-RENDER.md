...
> Guida passo-passo per spostare **Operazioni con le Frazioni** su
...
calcolo (m.c.m., fattorizzazione, semplificazione incrociata, verifica) avviene nel browser.
...
2. Dagli un nome (es. `operazioni-frazioni`), **Public** o **Private** a scelta.
...
git init
git add .
git commit -m "Operazioni con le Frazioni"
git branch -M main
git remote add origin https://github.com/<tuo-utente>/operazioni-frazioni.git
...
2. Apri l'URL fornito da Render (es. `https://operazioni-frazioni.onrender.com`).
3. Verifica che:
   ...
   - la **barra di accessibilità** compaia in alto (Font, Interlinea, Righello,
     Modalità, Ascolto).
